              ...
              <div class="card-body">
                ...
                <table id="table_pemasukan" class="table table-bordered table-striped">
                  ...
                  <?php
                        ...
                        $tahunSkrg = date('Y');
                        include '../koneksi.php';
                        $query = "SELECT * FROM `pembayaran` LEFT OUTER JOIN `client` ON `client`.`idClient` = `pembayaran`.`idClient` AND month(pembayaran.tanggalPembayaran) = '$bulanSkrg' AND year(pembayaran.tanggalPembayaran) = '$tahunSkrg' WHERE month(pembayaran.tanggalPembayaran) = '$bulanSkrg' AND client.status = '1' UNION SELECT * FROM `pembayaran` RIGHT OUTER JOIN `client` ON `client`.`idClient` = `pembayaran`.`idClient` AND month(pembayaran.tanggalPembayaran) = '$bulanSkrg' AND year(pembayaran.tanggalPembayaran) = '$tahunSkrg' WHERE client.status = '1'";
                        $i = 1;
